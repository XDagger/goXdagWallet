...
func CompareHashAndPassword(hashedPassword, password []byte) error {
	...
	otherHash, _, err := bcrypt(password, p.cost, p.salt)
	if err != nil {
		return err
	}

	otherP := &hashed{otherHash, p.salt, p.cost, p.major, p.minor, nil}
...
func newFromPassword(salt []byte, password []byte, cost int) (*hashed, error) {
	...
	hash, rawHash, err := bcrypt(password, p.cost, p.salt)
	if err != nil {
		return nil, err
	}
	p.hash = hash
...
func bcrypt(password []byte, cost int, salt []byte) ([]byte, []byte, error) {
...
func expensiveBlowfishSetup(key []byte, cost uint32, salt []byte) (*blowfish.Cipher, error) {
	...
	c, err := blowfish.NewSaltedCipher(key, csalt) // keep same result as java version
	if err != nil {
		return nil, err
	}
