...
/*
	...
	flag: 当前日志头部信息的标记位
*/
func NewXdagLog(out io.Writer, prefix string, flag int) *XdagLogger {

	//默认 debug打开， calledDepth深度为2,XdagLogger对象调用日志打印方法最多调用两层到达output函数
	zlog := &XdagLogger{out: out, prefix: prefix, flag: flag, file: nil, debugClose: false, calldDepth: 2}
	//设置log对象 回收资源 析构方法(不设置也可以，go的Gc会自动回收，强迫症没办法)
	runtime.SetFinalizer(zlog, CleanXdagLog)
	return zlog
}
...
/*
   回收日志处理
*/
func CleanXdagLog(log *XdagLogger) {
	log.closeFile()
}
...
/*
   制作当条日志数据的 格式头信息
*/
func (log *XdagLogger) formatHeader(buf *bytes.Buffer, t time.Time, file string, line int, level int) {
...
/*
   输出日志文件,原方法
*/
func (log *XdagLogger) OutPut(level int, s string) error {
...
// ====> Debug <====
func (log *XdagLogger) Debugf(format string, v ...interface{}) {
...
func (log *XdagLogger) Debug(v ...interface{}) {
	if log.debugClose == true {
		return
	}
...
// ====> Trace <====
func (log *XdagLogger) Trace(v ...interface{}) {
	_ = log.OutPut(LogTrace, fmt.Sprintln(v...))
}

// ====> Info <====
func (log *XdagLogger) Infof(format string, v ...interface{}) {
	_ = log.OutPut(LogInfo, fmt.Sprintf(format, v...))
}

func (log *XdagLogger) Info(v ...interface{}) {
	_ = log.OutPut(LogInfo, fmt.Sprintln(v...))
}

// ====> Warn <====
func (log *XdagLogger) Warnf(format string, v ...interface{}) {
	_ = log.OutPut(LogWarn, fmt.Sprintf(format, v...))
}

func (log *XdagLogger) Warn(v ...interface{}) {
	_ = log.OutPut(LogWarn, fmt.Sprintln(v...))
}

// ====> Error <====
func (log *XdagLogger) Errorf(format string, v ...interface{}) {
	_ = log.OutPut(LogError, fmt.Sprintf(format, v...))
}

func (log *XdagLogger) Error(v ...interface{}) {
	_ = log.OutPut(LogError, fmt.Sprintln(v...))
}

// ====> Fatal 需要终止程序 <====
func (log *XdagLogger) Fatalf(format string, v ...interface{}) {
	_ = log.OutPut(LogFatal, fmt.Sprintf(format, v...))
	os.Exit(1)
}

func (log *XdagLogger) Fatal(v ...interface{}) {
	_ = log.OutPut(LogFatal, fmt.Sprintln(v...))
	os.Exit(1)
}

// ====> Panic  <====
func (log *XdagLogger) Panicf(format string, v ...interface{}) {
	s := fmt.Sprintf(format, v...)
	_ = log.OutPut(LogPanic, s)
	panic(s)
}

func (log *XdagLogger) Panic(v ...interface{}) {
	s := fmt.Sprintln(v...)
	_ = log.OutPut(LogPanic, s)
	panic(s)
}
...
// ====> Stack  <====
func (log *XdagLogger) Stack(v ...interface{}) {
	s := fmt.Sprint(v...)
	s += "\n"
...
func (log *XdagLogger) ResetFlags(flag int) {
	log.mu.Lock()
	defer log.mu.Unlock()
	log.flag = flag
}
...
//添加flag标记
func (log *XdagLogger) AddFlag(flag int) {
	log.mu.Lock()
	defer log.mu.Unlock()
	log.flag |= flag
}
...
//设置日志的 用户自定义前缀字符串
func (log *XdagLogger) SetPrefix(prefix string) {
	log.mu.Lock()
	defer log.mu.Unlock()
	log.prefix = prefix
}
...
//设置日志文件输出
func (log *XdagLogger) SetLogFile(fileDir string, fileName string) {
	var file *os.File
...
func (log *XdagLogger) CloseDebug() {
	log.debugClose = true
}

func (log *XdagLogger) OpenDebug() {
...
//判断日志文件是否存在
func (log *XdagLogger) checkFileExist(filename string) bool {
	exist := true
